...
const devicesId = "642678301" // 填写在OneNet上获得的devicesId 形式就是一串数字 例子:9939133
const api_key = "ZjanXjXJFhttuTr=N2gbkoCUGtw=" // 填写在OneNet上的 api-key 例子: VeFI0HZ44Qn5dZO14AuLbWSlSlI=
...
Page({
  ...
  getDatapoints: function () {
    ...
    return new Promise((resolve, reject) => {
      wx.request({
        url: `https://api.heclouds.com/devices/${devicesId}/datapoints?datastream_id=time,distance&limit=20`,
        /**
         * 添加HTTP报文的请求头, 
        ...
        success: (res) => {
          ...
          that.setData({
            list:response.data.datastreams[1].datapoints[0].value
          })
          console.log(res.data)
          //console.log(response.data.datastreams[1].datapoints.reverse())
          ...
          resolve({
            ...
            time: response.data.datastreams[1].datapoints.reverse(),
          })
        },
        fail: (err) => {
          reject(err)
  ...
  update: function (datapoints) {
    ...
    this.lineChart_distance.updateData({
    ...
    this.lineChart_time.updateData({
      categories: wheatherData.categories,
      series: [{
        name: 'time',
        data: wheatherData.time,
        format: (val, name) => val.toFixed(2)
      }],
    })
  ...
  convert: function (datapoints) {
    ...
    for (var i = 0; i < length; i++) {
      ...
      time.push(datapoints.time[i].value);
    }
    return {
  ...
  firstDraw: function (datapoints) {
    ...
    //新建湿度图表
    this.lineChart_distance = new myCharts({
      canvasId: 'distance',
      type: 'line',
      categories: wheatherData.categories,
      animation: false,
    ...
    //新建温度图表
    this.lineChart_time = new myCharts({
      canvasId: 'time',
      type: 'line',
      categories: wheatherData.categories,
      animation: false,
